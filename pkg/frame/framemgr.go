...
import (
	...
	"github.com/esrrhs/pingtunnel/pkg/congestion"
...
func (fm *FrameMgr) WriteSendBuffer(data []byte) {
	...
	defer fm.sendblock.Unlock()
	fm.sendb.Write(data)
	//log.Printf("debugid %v WriteSendBuffer %v %v", fm.debugid, fm.sendb.Size(), len(data))
}
...
func (fm *FrameMgr) cutSendBufferToWindow(cur int64) {
	...
	for fm.sendb.Size() >= fm.frame_max_size && fm.sendwin.Size() < int(fm.windowsize) {
		...
		if err != nil {
			log.Printf("sendwin Set fail %v", err)
		}
		//log.Printf("debugid %v cut frame push to send win %v %v %v", fm.debugid, f.Id, fm.frame_max_size, fm.sendwin.Size())
	}
	...
	if sendall && fm.sendb.Size() > 0 && fm.sendwin.Size() < int(fm.windowsize) {
		...
		//log.Printf("debugid %v cut small frame push to send win %v %v %v", fm.debugid, f.Id, len(f.Data.Data), fm.sendwin.Size())
	}
	...
	if fm.sendb.Empty() && fm.close && !fm.closesend && fm.sendwin.Size() < int(fm.windowsize) {
		...
		fm.closesend = true
		//log.Printf("debugid %v close frame push to send win %v %v", fm.debugid, f.Id, fm.sendwin.Size())
	}
}
...
func (fm *FrameMgr) calSendList(cur int64) {
	...
	for e := fm.sendwin.FrontInter(); e != nil; e = e.Next() {
		...
		if !f.Acked && (f.Resend || cur-f.Sendtime > int64(fm.resend_timems*(int)(time.Millisecond))) &&
			...
			//log.Printf("debugid %v push frame to sendlist %v %v", fm.debugid, f.Id, len(f.Data.Data))
		}
	}
	fm.ctLastSendId = -1
}
	...
	for e := fm.recvlist.Front(); e != nil; e = e.Next() {
		...
		if f.Type == (int32)(Frame_REQ) {
			for _, id := range f.Dataid {
				tmpreq[id]++
				//log.Printf("debugid %v recv req %v %v", fm.debugid, f.Id, common.Int32ArrayToString(f.Dataid, ","))
			}
		} else if f.Type == (int32)(Frame_ACK) {
			for _, id := range f.Dataid {
				tmpack[id]++
				//log.Printf("debugid %v recv ack %v %v", fm.debugid, f.Id, common.Int32ArrayToString(f.Dataid, ","))
			}
			...
			//log.Printf("debugid %v recv data %v %v", fm.debugid, f.Id, len(f.Data.Data))
		} else if f.Type == (int32)(Frame_PING) {
			fm.processPing(f)
		} else if f.Type == (int32)(Frame_PONG) {
			fm.processPong(f)
		} else {
			log.Printf("error frame type %v", f.Type)
		}
	}
	fm.recvlist.Init()
...
func (fm *FrameMgr) processRecvList(tmpreq map[int32]int, tmpack map[int32]int, tmpackto map[int32]*Frame) {
	...
	for id, num := range tmpreq {
		...
		if err != nil {
			log.Printf("sendwin get id fail %v %v", id, err)
			continue
		}
		if value == nil {
			continue
		...
		if f.Id == id {
			f.Resend = true
			//log.Printf("debugid %v choose resend win %v %v", fm.debugid, f.Id, len(f.Data.Data))
		} else {
			log.Printf("sendwin get id diff %v %v", id, f.Id)
			continue
		}
		if fm.openstat > 0 {
	...
	for id, num := range tmpack {
		...
		if f.Id == id {
			f.Acked = true
			//log.Printf("debugid %v remove send win %v %v", fm.debugid, f.Id, len(f.Data.Data))
		} else {
			log.Printf("sendwin get id diff %v %v", id, f.Id)
			continue
		}
	...
	for !fm.sendwin.Empty() {
		...
		if err != nil {
			log.Printf("sendwin get Front fail %s", err)
			break
		}
		if value == nil {
			break
		...
		if f.Acked {
			...
			if err != nil {
				log.Printf("sendwin PopFront fail ")
				break
			}
		} else {
			break
		...
		for id, rf := range tmpackto {
			if fm.addToRecvWin(rf) {
				...
				if index >= tmpsize {
					...
					index = 0
					tmp = make([]int32, len(tmpackto))
					//log.Printf("debugid %v send ack %v %v", fm.debugid, f.Id, common.Int32ArrayToString(f.Dataid, ","))
				}
				//log.Printf("debugid %v add data to win %v %v", fm.debugid, rf.Id, len(rf.Data.Data))
			}
		}
		if index > 0 {
			f := &Frame{Type: (int32)(Frame_ACK), Resend: false, Sendtime: 0,
...
func (fm *FrameMgr) addToRecvWin(rf *Frame) bool {
	...
	if !fm.isIdInRange(rf.Id, fm.frame_max_id) {
		//log.Printf("debugid %v recv frame not in range %v %v", fm.debugid, rf.Id, fm.recvid)
	...
	if err != nil {
		log.Printf("recvwin Set fail %v", err)
		return false
	}
	return true
}
	...
	if f.Data.Type == (int32)(FrameData_USER_DATA) {
		...
		if left >= len(f.Data.Data) {
			...
			if f.Data.Compress {
				...
				if err != nil {
					log.Printf("recv frame deCompressData error %v", f.Id)
					return false
				}
				if left < len(old) {
					return false
				}
				//log.Printf("debugid %v deCompressData recv frame %v %v %v", fm.debugid, f.Id, len(src), len(old))
				src = old
			}
			...
			fm.recvb.Write(src)
			//log.Printf("debugid %v combined recv frame to recv buffer %v %v", fm.debugid, f.Id, len(src))
			return true
		}
		return false
	} else if f.Data.Type == (int32)(FrameData_CLOSE) {
		fm.remoteclosed = true
		//log.Printf("debugid %v recv remote close frame %v", fm.debugid, f.Id)
		return true
	} else if f.Data.Type == (int32)(FrameData_CONN) {
		fm.sendConnectRsp()
		fm.connected = true
		//log.Printf("debugid %v recv remote conn frame %v", fm.debugid, f.Id)
		return true
	} else if f.Data.Type == (int32)(FrameData_CONNRSP) {
		fm.connected = true
		//log.Printf("debugid %v recv remote conn rsp frame %v", fm.debugid, f.Id)
		return true
	} else if f.Data.Type == (int32)(FrameData_HB) {
		fm.lastRecvHBTime = time.Now().UnixNano()
		//log.Printf("debugid %v recv remote hb frame %v", fm.debugid, f.Id)
		return true
	} else {
		log.Printf("recv frame type error %v", f.Data.Type)
		return false
	}
}
...
func (fm *FrameMgr) combineWindowToRecvBuffer(cur int64) {
		...
		if err == nil {
			...
			if f.Id == fm.recvid {
				...
				if fm.processRecvFrame(f) {
					...
					if err != nil {
						log.Printf("recvwin PopFront fail %v ", err)
					}
					done = true
					//log.Printf("debugid %v process recv frame ok %v %v", fm.debugid, f.Id, len(f.Data.Data))
				}
			}
		}
		if !done {
			//log.Printf("debugid %v combined need recvid %v ", fm.debugid, fm.recvid)
			break
		} else {
			fm.recvid++
			...
			//log.Printf("debugid %v combined ok add recvid %v ", fm.debugid, fm.recvid)
		}
	}
	...
	for len(reqtmp) < int(fm.windowsize) && len(reqtmp)*4 < fm.frame_max_size/2 && e != nil {
		f := e.Value.(*Frame)
		//log.Printf("debugid %v start add req id %v %v %v", fm.debugid, fm.recvid, f.Id, id)
		if f.Id != id {
			oldReq := fm.reqmap[f.Id]
			if cur-oldReq > fm.rttns {
				reqtmp[id]++
				fm.reqmap[f.Id] = cur
				//log.Printf("debugid %v add req id %v ", fm.debugid, id)
			}
		} else {
			e = e.Next()
		}
	...
	if len(reqtmp) > 0 {
		...
		fm.sendFrame(f)
		//log.Printf("debugid %v send req %v %v", fm.debugid, f.Id, common.Int32ArrayToString(f.Dataid, ","))
	}
}
...
func (fm *FrameMgr) GetRecvReadLineBuffer() []byte {
	...
	//log.Printf("debugid %v GetRecvReadLineBuffer %v %v", fm.debugid, fm.recvb.Size(), len(ret))
	return ret
}
...
func (fm *FrameMgr) SkipRecvBuffer(size int) {
	...
	//log.Printf("debugid %v SkipRead %v %v", fm.debugid, fm.recvb.Size(), size)
}
...
func (fm *FrameMgr) ping() {
	...
	if cur-fm.lastPingTime > (int64)(time.Second) {
		...
		f := &Frame{Type: (int32)(Frame_PING), Resend: false, Sendtime: cur,
			Id: 0}
		fm.sendFrame(f)
		//log.Printf("debugid %v send ping %v", fm.debugid, cur)
		if fm.openstat > 0 {
			fm.fs.sendping++
		}
	}
...
func (fm *FrameMgr) hb() {
	...
	if cur-fm.lastSendHBTime > (int64)(time.Second) && fm.sendwin.Size() < int(fm.windowsize) {
		...
		//log.Printf("debugid %v send hb %v", fm.debugid, f.Id)
	}
}
...
func (fm *FrameMgr) processPing(f *Frame) {
	...
	//log.Printf("debugid %v recv ping %v", fm.debugid, f.Sendtime)
}
...
func (fm *FrameMgr) processPong(f *Frame) {
	...
	if cur > f.Sendtime {
		...
		fm.lastPongTime = cur
		//log.Printf("debugid %v recv pong %v %dms", fm.debugid, rtt, fm.rttns/1000/1000)
	}
}
...
func (fm *FrameMgr) Connect() {
	if fm.sendwin.Size() < int(fm.windowsize) {
		...
		//log.Printf("debugid %v start connect", fm.debugid)
	} else {
...
func (fm *FrameMgr) sendConnectRsp() {
	if fm.sendwin.Size() < int(fm.windowsize) {
		...
		//log.Printf("debugid %v send connect rsp", fm.debugid)
	} else {
		log.Printf("sendConnectRsp fail ")
	}
}
